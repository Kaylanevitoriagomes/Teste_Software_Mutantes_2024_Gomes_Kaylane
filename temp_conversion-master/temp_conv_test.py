...
def test_c_to_k_correct():
    params = {'input': '-45.14 celsius', 'response': 228.15, 'unit': 'kelvin'}
    tc = TempConvertor(params)
    tc.check_response_to_answer()
    assert round(tc.answer) == round(tc.response)
    assert tc.is_response_correct == "correct"
...
def test_k_to_c_correct():
    params = {'input': '300 kelvin', 'response': 26.85, 'unit': 'celsius'}
    tc = TempConvertor(params)
    tc.check_response_to_answer()
    assert round(tc.answer, 2) == round(tc.response, 2)
    assert tc.is_response_correct == "correct"
